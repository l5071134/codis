...
static long
numberOfPayloadMessagesFromObject(robj *val, long long maxbulks) {
...
static long
estimateNumberOfPayloadMessages(redisDb *db, robj *key, long long maxbulks) {
    robj *val = lookupKeyWrite(db, key);
    if (val != NULL) {
        return numberOfPayloadMessagesFromObject(val, maxbulks);
    }
    return 0;
}
...
singleObjectIteratorNext(client *c, singleObjectIterator *it,
    ...
    if (it->stage == STAGE_PREPARE) {
        ...
        if (numberOfPayloadMessagesFromObject(val, maxbulks) != 1) {
...
batchedObjectIteratorAddKey(redisDb *db, batchedObjectIterator *it, robj *key) {
    ...
    it->estimate_msgs += estimateNumberOfPayloadMessages(db, key, it->maxbulks);
...
batchedObjectIteratorAddKeyCallback(void *data, const dictEntry *de) {
    ...
    long msgs = estimateNumberOfPayloadMessages(db, key, it->maxbulks);
